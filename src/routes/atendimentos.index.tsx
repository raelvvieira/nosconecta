...
import { ArrowLeft, CheckCheck, ChevronDown, RefreshCw, Search, Send, WifiOff } from "lucide-react";
...
function ChatPage() {
  ...
  return (
    <main className="mx-auto flex w-full flex-1 flex-col pb-20 lg:h-full lg:max-w-[1400px] lg:flex-row lg:overflow-hidden lg:pb-0">
          ...
          {!connected ? (
            <div className="grid min-h-52 place-items-center px-6 text-center">
              <div>
                ...
                <h3 className="mt-4 font-semibold">WhatsApp não conectado</h3>
                <p className="mt-1 text-sm text-muted-foreground">Conecte pra ver as conversas aqui.</p>
                <Button
      ...
      <section className={cn("flex min-w-0 flex-1 flex-col", !conversationId && "hidden lg:flex")}>
        {!selected ? (
          <div className="hidden flex-1 items-center justify-center lg:flex">
            <p className="text-sm text-muted-foreground">
              {connected ? "Selecione uma conversa para ver as mensagens." : "Conecte o WhatsApp pra começar a atender."}
            </p>
          </div>
        ) : (
          <>
      ...
      <WhatsappConnectSheet open={connectSheetOpen} onOpenChange={setConnectSheetOpen} />
    </main>
  );
}

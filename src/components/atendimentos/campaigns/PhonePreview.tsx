      ...
      <div className="overflow-hidden rounded-[32px] border-[6px] border-foreground bg-[#ECE5DD] shadow-soft">
        <div className="flex items-center gap-2 bg-gradient-primary px-3 py-2.5">
          ...
          <div className="min-w-0">
            <p className="truncate text-[11px] font-semibold text-white">Contato</p>
        ...
        <div className="flex min-h-[220px] flex-col justify-end p-2.5">
          {content.trim() || mediaUrl ? (
            <div className={cn("max-w-[85%] self-end rounded-2xl bg-gradient-primary px-2.5 py-2 text-white shadow-soft")}>
              ...
              {content.trim() && <p className="whitespace-pre-wrap break-words text-[11px] leading-4">{preview}</p>}
            ...
            <p className="text-center text-[11px] text-muted-foreground">A mensagem aparecerá aqui</p>
          )}
        </div>
        <div className="border-t border-black/5 bg-white px-2.5 py-2">
          <div className="rounded-full bg-muted px-3 py-1.5 text-[10px] text-muted-foreground">Mensagem</div>
        </div>
      </div>
    </div>
  );

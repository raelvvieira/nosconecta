...
async function enviar(
  ...
  // Caminho ESPECULATIVO, para quem nunca conversou com a clínica. O endpoint
  // de ações agendadas existe e já é usado com `conversation_id`; o campo
  // `contact_id` viaja no payload e há um `by_contact/{id}`, mas nada prova que
  // ele funcione sozinho. Se o CRM recusar, o erro fica gravado no alvo — é
  // assim que a dúvida vira resposta depois do primeiro lote real.
  await crmFetch(supabase, ownerId, "/api/v1/scheduled_actions", {
    method: "POST",
    body: JSON.stringify({
